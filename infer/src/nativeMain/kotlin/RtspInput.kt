...
class RtspInput(val url: String) : Video {
    ...
    override fun frames() = flow {
        memScoped {
            val deviceRef = alloc<CPointerVar<AVBufferRef>>()
            Device(deviceRef).use { device ->
                ...
                try {
                    ...
                    try {
                        ...
                        avcodec_open2(codecCtx.value, codec, null).check("avcodec_open2")
                        ToRGBImage().apply { init(codecCtx.pointed!!) }.use { toRGBImage ->
                                    ...
                                    try {
                                        if (packet.pointed!!.stream_index == videoStreamIndex) {
                                            ...
                                            while (avcodec_receive_frame(codecCtx.value, frame.value) == 0) {
                                                ...
                                                emit(Video.Frame(timestamp.seconds, toRGBImage(frame.pointed!!)))
                                            }
                                        }
                                    } finally {
                                        av_packet_unref(packet.value)
                                    }
                                    if (packet.value == null) break
                                }
                            } finally {
